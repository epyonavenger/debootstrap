...
if [[ ! -f "./includes/answers.env" ]] || [[ $PRE_INSTALL_COMPLETE -ne "true" ]]; then
    echo "## Answers file is missing or incomplete, please re-run 00_start_here.sh ##"
    exit 1
else
    source includes/answers.env
fi
...
cp includes/ubuntu.sources /mnt/etc/apt/sources.list.d/ubuntu.sources
cp includes/ignored-packages /mnt/etc/apt/preferences.d/ignored-packages
...
if "$DESKTOP_INSTALL"; then
    ...
    cp includes/mozilla.list /mnt/etc/apt/sources.list.d/mozilla.list
    cp includes/mozilla /mnt/etc/apt/preferences.d/mozilla
    # Stage Google Chrome.
